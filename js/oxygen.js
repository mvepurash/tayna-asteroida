...
const Oxygen = (() => {
  ...
  function reset() {
    current  = max;
    depleted = false;
    // ВАЖНО: снимаем паузу. reset() вызывается только когда рейс начинается
    // заново (новая игра, респавн, воскрешение за рекламу), и кислород обязан
    // тикать. Без этой строки был баг: если игрок свернул вкладку на экране
    // смерти, обработчик ставил кислород на паузу, а возобновлял только при
    // активной игре — и после «Попробовать снова» счётчик оставался замороженным.
    paused   = false;
    _clearPanic();
  }
