...
const Astronaut = (() => {
  ...
  // 1.8с — 5 кадров при fps≈2.78. Было 5.0с (1 кадр в секунду): выход из
  // шаттла тянулся так долго, что замерший на это время счётчик кислорода
  // выглядел сломанным, особенно при перезапуске после гибели экипажа.
  // ВАЖНО: должно совпадать с fps анимации spawn в renderer.js (5 / 1.8 ≈ 2.78).
