...
const UIManager = (() => {
  ...
  const BUTTONS = {
    ...
    settings: [  // ТОЧНЫЙ скан границ 17.09.2026
      ...
      { id: 'lang',  x: 283, y: 434, w: 114, h: 32 },  // выбор языка (точный скан рамки кнопки 283..396; x=425 — это край СТРОКИ, не кнопки)
      { id: 'reset', x: 72,  y: 500, w: 336, h: 55 },  // СБРОС ПРОГРЕССА (двойной клик). Точный замер по срезу вне текста: 72..407, 500..554. Прошлые правки брали высоту 73 — зона вылезала на 19px ниже строки
  ...
  function _drawSettingsExtras(ctx, dt) {
    ...
    if (_resetArm > 0) {
      const X = 72, Y = 500, W = 336, H = 55;   // те же границы, что у зоны нажатия
  ...
  // Координаты кнопок, отличающиеся в локализованных версиях экранов.
  // Обычно художник делает перевод поверх русского макета, и зоны совпадают —
  // тогда сюда ничего дописывать не нужно. Но на экране инструктажа английская
  // панель нарисована крупнее (занимает 83% высоты кадра против 63% у русской),
  // поэтому кнопки там стоят в других местах. Растягивать картинку под русскую
  // геометрию нельзя — исказится текст, проще указать фактические координаты.
  // Перезагрузить картинки экранов — вызывается при смене языка игроком.
  // Старые объекты Image остаются на месте до прихода новых, поэтому текущий
  // экран не мигает и не пропадает: рисуется то, что уже загружено.
  function reloadScreens() {
    const names = ['title_screen', 'pause_screen', 'settings_screen', 'game_over_screen', 'briefing_screen', 'records_screen', 'pause_button'];
    let loaded = 0;
    console.log('[UI] перезагрузка экранов под язык:', I18n.getScreenLang());
    names.forEach(n => _loadScreen(n, () => {
      loaded++;
      if (loaded === names.length) console.log('[UI] экраны перезагружены');
    }));
  }

  const BUTTONS_BY_LANG = {
    en: {
      howto: [
        { id: 'back', x: 131, y: 732, w: 208, h: 53 },          // GOT IT
        { id: 'back', x: 401, y: 62,  w: 37,  h: 40, pad: 8 },  // X (закрыть)
        { id: 'back', x: 0,   y: 0,   w: 480, h: 854 },         // запасная зона
      ],
    },
  };

  function _buttonsFor(st) {
    const lang = (typeof I18n !== 'undefined') ? I18n.getScreenLang() : 'ru';
    const byLang = BUTTONS_BY_LANG[lang];
    if (byLang && byLang[st]) return byLang[st];
    return BUTTONS[st] || [];
  }
  ...
  function _onButton(id) {
    ...
    switch (id) {
      ...
      case 'lang':
        // Переключение по кругу: при двух языках работает как тумблер.
        // Подпись на самом экране («РУССКИЙ» / «ENGLISH») меняется вместе с
        // картинкой, поэтому отдельно её рисовать не нужно.
        I18n.cycle();
        reloadScreens();
        break;
      case 'records':
