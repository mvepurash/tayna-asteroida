...
const UIManager = (() => {
  ...
  // ⚠️ ВРЕМЕННО НА ПЕРИОД КАЛИБРОВКИ КНОПОК (запрос 13.09.2026) ⚠️
  // Значения увеличены в ~12 раз, чтобы успевать делать скриншот вспышки.
  // ПОСЛЕ ЗАВЕРШЕНИЯ КАЛИБРОВКИ ВЕРНУТЬ: FLASH_DURATION=0.16, PRESS_DELAY_MS=120
  const FLASH_DURATION = 2.0;   // было 0.16
  const PRESS_DELAY_MS = 2000;  // было 120 — задержка действия после клика, даём вспышке доиграть на ТОМ ЖЕ экране
