...
const Game = (() => {
  ...
  function init() {
    ...
    Input.init();
    // Сигнал готовности Яндексу шлём ТОЛЬКО когда картинки экранов загружены —
    // иначе игрок увидит наш индикатор "ЗАГРУЗКА…" вместо лоадера платформы.
    UIManager.init(_notifyReady);
    ...
    // Страховка: если какая-то картинка не догрузится и коллбек не сработает,
    // всё равно рапортуем через 8 секунд — иначе лоадер зависнет навсегда.
    setTimeout(_notifyReady, 8000);
  }
  ...
  function _loopInner(timestamp) {
    ...
    if (!UIManager.isPlaying()) {
      ...
      try { UIManager.draw(_ctx(), dt); } catch(e) { console.error("[UIManager.draw]", e); }
      requestAnimationFrame(loop);
      return;
    }
    ...
    try { UIManager.draw(_ctx(), dt); } catch(e) { console.error("[UIManager.draw]", e); }  // кнопка паузы поверх игры
    requestAnimationFrame(loop);
  }
